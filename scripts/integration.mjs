...
export function dxBasedLevel(actor, skill) {
  const level=Number(skill.level);if(!Number.isFinite(level))throw new Error('The imported skill has no numeric level.');
  const relative=String(skill.relativelevel??'').match(/^(ST|DX|IQ|HT|WILL|PER)\s*([+-]\s*\d+)?$/i);
...
export function participantFromActor(actor, skill, side='quarry') {
  ...
  const hasGunslinger=tree=>Object.values(tree??{}).some(x=>x&&typeof x==='object'&&(/^gunslinger(?:$|\s*\()/i.test(String(x.name??'').trim())||hasGunslinger(x.contains)||hasGunslinger(x.collapsed)));
  const selected=skill??find('running');
  return newParticipant({name:actor.name,actorUuid:actor.uuid,side,skillName:selected?`${selected.name} (DX-based; review)`:'DX',
    skill:selected?dxBasedLevel(actor,selected):numeric(sys.attributes?.DX?.value)??12,
    gunslinger:hasGunslinger(sys.ads),
    footSkill:find('running')?dxBasedLevel(actor,find('running')):numeric(sys.attributes?.DX?.value),
    footSkillName:find('running')?'Running (DX-based)':'DX',
    speed:numeric(sys.currentmove)??numeric(sys.basicmove?.value)??5,
    st:numeric(sys.attributes?.ST?.value),ht:numeric(sys.attributes?.HT?.value),hp:numeric(sys.HP?.value),maxHp:numeric(sys.HP?.max),
    ...
    notes:'Actor snapshot: check DX-based skill, top speed (including Enhanced Move), and any situational bonuses. Changes here do not update the actor.'});
}
export function actorOwned(uuid,user) {
  if(!uuid)return false;
  try {return !!globalThis.fromUuidSync?.(uuid)?.testUserPermission(user,'OWNER');} catch {return false;}
}
export async function rollGGA(s,p,otf,event={}) {
  if(!canControl(s,p,game.user,actorOwned))throw new Error('You do not control this participant.');
  if(!p.actorUuid)throw new Error('This participant has no linked actor. Use the manual skill roll instead.');
  const actor=await fromUuid(p.actorUuid);
  if(!actor?.testUserPermission(game.user,'OWNER'))throw new Error('Native GGA rolls require ownership of the linked actor. Use an ad hoc roll or ask the GM.');
  ...
  if(!action||!['attack','skill-spell','attribute','controlroll','damage','deriveddamage','attackdamage'].includes(action.type))throw new Error('Use a single attack, skill, attribute, defence, or damage expression. Chat commands are not accepted here.');
...
export async function standaloneRoll({name,target,modifier=0,whisper=false}) {
  ...
  return roll.toMessage({speaker:{alias:textValue(name,100)},flavor:`${escapeHTML(name)} · ${roll.total} vs ${effective} · margin ${effective-roll.total} (ad hoc roll; GM resolves consequences)`,
    ...(whisper?{whisper:ChatMessage.getWhisperRecipients('GM').map(u=>u.id)}:{})},{rollMode:whisper?'gmroll':'publicroll'});
}
