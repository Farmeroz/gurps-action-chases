...
export function number(value, label, min = -100, max = 10000, integer = false) {
  if (value === '' || value == null || !Number.isFinite(Number(value)))
    throw new Error(`${label} needs a number.`);
...
export function textValue(value, max = 300) {
  return String(value ?? '')
    .trim()
    .slice(0, max);
}
export const MANOEUVRES = {
  move: {
    ...
    mod: 0,
    page: 33,
    hint: 'Record the declaration, then follow any tasks before Chase Rolls.',
  },
  attack: {
    name: 'Attack',
    sides: SIDES,
    ...
    static: true,
    page: 32,
    hint: 'Open the attack task and review its preview before recording the shot.',
  },
  moveAttack: {
    ...
    mod: 0,
    page: 33,
    hint: 'For a vehicle, confirm whether the driver also attacks. Resolve the offered attack tasks.',
  },
  embark: {
    ...
    page: 32,
    scenery: true,
    hint: 'Enter or leave a vehicle. Use the new travel mode next round.',
  },
  emergency: {
    ...
    mod: -5,
    page: 32,
    hint: 'Follow the recovery prompt and check the participant’s due round.',
  },
  force: {
    name: 'Force',
    sides: SIDES,
    ...
    vehicle: true,
    page: 32,
    hint: 'Resolve hit, vehicular dodge, and target control roll before the contest.',
  },
  hide: {
    name: 'Hide',
    sides: ['quarry'],
    ...
    page: 32,
    scenery: true,
    hint: 'Confirm the scene and supply any requested skill. Review the offered outcome after both rolls.',
  },
  mobilityEscape: {
    ...
    page: 32,
    scenery: true,
    hint: 'GM decides whether the pursuer can follow. If unable, mark the pursuer as forced static.',
  },
  mobilityPursuit: {
    ...
    page: 33,
    scenery: true,
    hint: 'Ask the GM to confirm the route, then use the offered outcome controls.',
  },
  ram: {
    name: 'Ram',
    sides: ['pursuer'],
    ...
    vehicle: true,
    page: 33,
    hint: 'Complete the GM contact task, including defence, damage and relevant control results.',
  },
  reverse: {
    name: 'Reverse',
    sides: ['quarry'],
    mod: -10,
    page: 33,
    hint: 'If prompted, resolve the original Chase Roll result before selecting the outcome; do not reroll it.',
  },
  stop: {
    name: 'Stop',
    sides: SIDES,
    mod: 0,
    page: 33,
    hint: 'End the chase: pursuer lets quarry go; quarry stops for interaction or combat.',
  },
  stunt: {
    name: 'Stunt',
    sides: SIDES,
    mod: 0,
    page: 33,
    hint: 'Enter the agreed risk and description. Record every required stunt task, then review the updated target.',
  },
  stuntEscape: {
    name: 'Stunt Escape',
    sides: ['quarry'],
    ...
    page: 34,
    scenery: true,
    hint: 'Confirm the scene with the GM, record the stunt tasks and follow any pursuit or consequence prompts.',
  },
};
...
export function successRoll(target, total) {
  ...
  const criticalSuccess =
    total <= 4 || (total === 5 && target >= 15) || (total === 6 && target >= 16);
  const criticalFailure = total === 18 || (total === 17 && target <= 15) || total - target >= 10;
  return {
    target,
    total,
    margin: target - total,
    success: criticalSuccess || (total <= target && total < 17),
    criticalSuccess,
    criticalFailure,
  };
}
...
export function newParticipant(raw = {}) {
  ...
  const out = {
    id: raw.id || uid(),
    kind,
    name: textValue(raw.name || (kind === 'vehicle' ? 'New vehicle' : 'New character'), 100),
    side: raw.side === 'pursuer' ? 'pursuer' : 'quarry',
    actorUuid: textValue(raw.actorUuid, 200),
    controllers: Array.isArray(raw.controllers)
      ? [...new Set(raw.controllers.map((x) => textValue(x, 100)))].slice(0, 50)
      : [],
    skillName: textValue(
      raw.skillName || (kind === 'vehicle' ? 'Driving' : 'Running (DX-based)'),
      100,
    ),
    skill: number(raw.skill ?? 12, 'DX-based chase skill', 1, 1000),
    speed: number(raw.speed ?? (kind === 'vehicle' ? 30 : 5), 'Top speed', 0, 1e12),
    handling: number(raw.handling ?? 0, 'Handling', -20, 20),
    transportId: textValue(raw.transportId, 100),
    role: raw.role === 'operator' ? 'operator' : 'passenger',
    ...
    notes: textValue(raw.notes, 1500),
    gunslinger: !!raw.gunslinger,
    driverRequired: kind === 'vehicle' && !!raw.driverRequired,
    footMode: !!raw.footMode,
    footSkillName: textValue(raw.footSkillName || 'Running (DX-based)', 100),
  };
  for (const key of [
    'sr',
    'st',
    ...
    out[key] =
      raw[key] === '' || raw[key] == null
        ? null
        : number(raw[key], key, key === 'hp' || key === 'sm' ? -10000 : 0, 1000000);
  return out;
}
export function freshSide(leader = '') {
  return {
    leader,
    declaration: null,
    roll: null,
...
export function newChase(name = 'Untitled chase') {
  return {
    ...
    blind: false,
    participants: [],
    sides: { quarry: freshSide(), pursuer: freshSide() },
    passengerActions: {},
    checks: [],
    log: [],
    result: null,
    ...
    blindRequests: {},
    workflowVersion: 3,
    workflow: { approved: {}, outcomes: {}, legacyResolved: false },
    pendingCrew: [],
    pendingLeaders: {},
    outcomeRequired: '',
    ruleOptions: { speedRounding: 'action', driverShots: 'general' },
  };
}
export const unit = (s, id) => s.participants.find((p) => p.id === id);
export const isActive = (p) => p && !['wreck', 'out'].includes(p.status);
export const roots = (s, side) =>
  s.participants.filter((p) => p.side === side && !p.transportId && isActive(p));
export function operatorOf(s, p) {
...
export const missingDriver = (s, p) =>
  p?.kind === 'vehicle' &&
  isActive(p) &&
  !operatorOf(s, p) &&
...
export const rollsStarted = (s) =>
  SIDES.some((k) => s.sides[k].roll) || ['range', 'reverseCheck'].includes(s.phase);
...
export const footSkillOf = (p) =>
  p?.footSkill ?? (/^(running\b|dx\b)/i.test(p?.skillName ?? '') ? p.skill : null);
export function travelStats(s, p) {
  if (!p) throw new Error('Choose a leader for each side.');
  if (missingDriver(s, p))
    return { ...p, skill: null, skillName: 'No active driver', actorUuid: '' };
  if (p.kind === 'character' && !p.transportId && p.footMode && p.footSkill != null)
    return { ...p, skill: p.footSkill, skillName: p.footSkillName };
  const operator = operatorOf(s, p);
  return {
    ...p,
    skill: operator?.skill ?? p.skill,
    skillName: operator?.skillName ?? p.skillName,
    actorUuid: operator?.actorUuid || p.actorUuid,
  };
}
export function canControl(s, p, user, actorOwned = () => false) {
  ...
  return (
    p.controllers.includes(user.id) ||
    actorOwned(p.actorUuid, user) ||
    !!(op && (op.controllers.includes(user.id) || actorOwned(op.actorUuid, user)))
  );
}
export function manoeuvreError(s, side, key) {
  ...
  if (m.close && s.band !== 0)
    return 'This manoeuvre requires Close range at the start of the round.';
  ...
  if (due && !['emergency', 'stop'].includes(key))
    return 'The leader must take Emergency Action or Stop after the close call.';
  if (key === 'emergency' && !due)
    return 'Emergency Action requires a close call or a GM-recorded takeover requiring recovery.';
  return '';
}
export function validateDeclaration(s, side, raw) {
  const err = manoeuvreError(s, side, raw.key);
  ...
  return {
    key: raw.key,
    penalty,
    operatorAttacks: !!raw.operatorAttacks,
    notes: textValue(raw.notes, 500),
  };
}
export function breakdown(s, side) {
  if (missingDriver(s, unit(s, s.sides[side].leader)))
    throw new Error('Choose a replacement driver or resolve the vehicle’s wreck.');
  const team = s.sides[side],
    p = travelStats(s, unit(s, team.leader)),
    d = team.declaration;
  ...
  if (base == null)
    throw new Error('Enter a DX-based Stealth level or a base-skill override for Hide.');
  let mod = MANOEUVRES[key].mod,
    pendingStunt = false;
  ...
  const parts = [
    ...
    {
      label: 'Speed',
      value: isStatic ? 0 : speedBonus(p.speed, s.ruleOptions?.speedRounding !== 'table'),
    },
    { label: 'Handling', value: p.kind === 'vehicle' ? p.handling : 0 },
    { label: MANOEUVRES[key].name, value: mod },
    { label: 'Complementary skill', value: team.comp },
    { label: 'Other modifiers', value: team.extra },
  ];
  return { parts, target: parts.reduce((n, x) => n + x.value, 0), isStatic, pendingStunt };
}
export function contest(s) {
  const q = s.sides.quarry,
    p = s.sides.pursuer;
  ...
  const margin = Math.abs(diff),
    steps = margin >= 10 ? 2 : margin >= 5 ? 1 : 0;
  // Criticals do not automatically win a Quick Contest: Basic Set p. B348.
  if (q.declaration.key === 'hide')
    return {
      type: winner === 'quarry' ? 'hideWin' : 'fixed',
      winner,
      margin,
      steps: 0,
      band: winner === 'quarry' ? null : 0,
    };
  if (q.declaration.key === 'reverse') {
    ...
    return {
      ...
      steps: 0,
      band: 0,
      swap: winner === 'quarry' && q.roll.success && !wiped,
    };
  }
  const qs = breakdown(s, 'quarry').isStatic,
    ps = breakdown(s, 'pursuer').isStatic;
...
export function rangeChoices(s, result) {
  if (result.type === 'hideWin')
    return [
      { key: 'escape', label: 'Escape' },
      ...[0, 1, 2].map((band) => ({
        key: `hide-${band}`,
        label: `Become pursuer at ${BANDS[band]}`,
        band,
        swap: true,
      })),
    ];
  if (result.type === 'fixed' || result.type === 'reverse')
    return [
      {
        key: 'fixed',
        label: `${BANDS[result.band]}${result.swap ? ' · swap roles' : ''}`,
        band: result.band,
        swap: result.swap,
      },
    ];
  ...
  const mustClose =
    result.winner === 'pursuer' && s.sides.pursuer.declaration.key === 'mobilityPursuit';
  for (let n = 1; n <= result.steps; n++) {
    if (s.band - n >= 0)
      choices.push({
        key: `close-${n}`,
        label: `Close ${n} → ${BANDS[s.band - n]}`,
        band: s.band - n,
      });
    if (!mustClose && s.band + n <= 4)
      choices.push({
        key: `open-${n}`,
        label: `Open ${n} → ${BANDS[s.band + n]}`,
        band: s.band + n,
      });
    if (
      !mustClose &&
      result.winner === 'quarry' &&
      s.band + n > 4 &&
      !choices.some((x) => x.key === 'escape')
    )
      choices.push({ key: 'escape', label: 'Escape beyond Extreme' });
  }
  return choices;
}
export function attackModifiers({
  ...
  if ((!ordinary && !gunslinger) || manoeuvre === 'hide')
    throw new Error('This manoeuvre does not allow this ranged attack.');
  ...
  const accuracy =
    manoeuvre === 'attack' ? acc + (gunslinger ? 1 : 0) : gunslinger && moving ? acc : 0;
  return {
    range: RANGE_PENALTIES[band],
    movement,
    accuracy,
    target,
    total: RANGE_PENALTIES[band] + movement + accuracy + target,
  };
}
