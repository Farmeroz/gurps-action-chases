...
export const readChase = (doc) => {
  const data = doc?.getFlag(ID, 'data');
  ...
  try {
    ...
    const recovery = recoveryState(
      data,
      'The saved chase is not valid JSON. Restore a backup or seek help with the downloaded original record.',
    );
    recovery.name = String(doc?.name ?? 'Saved chase needs recovery').replace(/^Chase · /, '');
    return recovery;
  }
  return s?.schema === 1 ? upgrade(s) : null;
};
export const chaseDocuments = () =>
  [...game.journal].filter((d) => readChase(d) && d.testUserPermission(game.user, 'OBSERVER'));
export const activeGM = () =>
  game.users.activeGM ??
  [...game.users].filter((u) => u.active && u.isGM).sort((a, b) => a.id.localeCompare(b.id))[0];
...
export async function createChase(name, imported = null, initial = null) {
  ...
  return JournalEntry.create({
    ...
    flags: { [ID]: { data: JSON.stringify(s) } },
    pages: [
      {
        name: 'About this chase',
        type: 'text',
        text: {
          content:
            '<p>Open this chase using the Action Chases toolbar button, Combat sidebar button, or Ctrl+Shift+C. Chase data is shared with players and survives clearing chat. Export a JSON backup from the Chase window.</p>',
        },
      },
    ],
  });
...
const pending = new Map(),
  feedback = new Map(),
  privateChoices = new Map();
let refreshUI = () => {};
export const submissionStatus = (docId) => feedback.get(docId);
export const ownCommitment = (docId, epoch, side) =>
  privateChoices.get(`${docId}:${epoch}:${side}`);
function status(docId, kind, text) {
  feedback.set(docId, { kind, text });
  refreshUI();
}
export function submit(doc, command) {
  const s = readChase(doc);
  if (!s) return Promise.reject(new Error('This chase no longer exists.'));
  const gm = activeGM();
  if (!gm)
    return Promise.reject(new Error('An active GM is required. Your entries have been kept.'));
  const cmd = {
    ...command,
    epoch: command.epoch ?? s.epoch,
    revision: command.revision ?? s.revision,
  };
  const key = JSON.stringify([
    ...
    cmd.epoch,
    cmd.type,
    cmd.taskId ?? cmd.side ?? cmd.id ?? cmd.data?.id ?? '',
  ]);
  if (pending.has(key)) {
    const prior = pending.get(key);
    return prior.command === JSON.stringify(cmd)
      ? prior.promise
      : Promise.reject(
          new Error(
            'Another submission for this task is still waiting. Your entries have been kept.',
          ),
        );
  }
  let resolve, reject;
  ...
  entry.finish = (ok, error, value) => {
    ...
    pending.delete(key);
    if (ok && s.blind && cmd.type === 'declare')
      privateChoices.set(`${doc.id}:${s.epoch}:${cmd.side}`, cmd.data.key);
    status(doc.id, ok ? 'accepted' : 'rejected', ok ? 'Accepted. Chase updated.' : error);
    if (ok) resolve(value ?? { accepted: true });
    else reject(new Error(error));
  };
  status(doc.id, 'pending', 'Sending to the active GM…');
  void (async () => {
    try {
      if (isAuthority() && !(s.blind && cmd.type === 'declare'))
        return entry.finish(true, '', await queue(() => execute(doc, cmd, game.user)));
      // Receipts are accepted only from the authenticated GM update hook below.
      entry.timer = setTimeout(
        () =>
          entry.finish(
            false,
            'No acknowledgement yet. Your entries are kept. Check the chase and the request in chat before trying again.',
          ),
        25000,
      );
      ...
      entry.finish(false, err.message);
    }
  })();
  return promise;
}
export async function showToPlayers(doc) {
  ...
  return ChatMessage.create({
    user: game.user.id,
    content: `<p><strong>${escapeHTML(readChase(doc).name)}</strong></p><button type="button" data-gac-open="${escapeHTML(doc.id)}">Open chase tracker</button>`,
    flags: { [ID]: { invitation: { docId: doc.id } } },
  });
}
async function commitment(request) {
  const bytes = new TextEncoder().encode(
    JSON.stringify({ data: request.command.data, nonce: request.nonce }),
  );
...
export async function execute(doc, command, user, requestId = '') {
  ...
  if (!state) throw new Error('Chase data is missing.');
  ...
  let roll = null,
    privateDeclarations = null;
  if (command.type === 'reveal') {
    privateDeclarations = {};
    for (const side of SIDES) {
      const ref = state.blindRequests[side],
        message = game.messages.get(typeof ref === 'string' ? ref : ref?.id),
        request = message?.getFlag(ID, 'request');
      if (
        !request ||
        request.command.epoch !== state.epoch ||
        request.command.side !== side ||
        !ref.hash ||
        (await commitment(request)) !== ref.hash
      )
        throw new Error(
          'A private commitment is missing or changed. Undo the commitments and submit them again.',
        );
  ...
  if (command.type === 'declare' && state.blind) {
    ...
    if (JSON.stringify(request.command) !== JSON.stringify(command))
      throw new Error('The declaration was edited before acceptance. Submit it again.');
  ...
  if (roll) {
    ...
    if (command.type === 'roll') {
      const p = unit(state, state.sides[command.side].leader),
        b = breakdown(state, command.side);
      speaker = p.name;
      ...
      const o = next.workflow.outcomes[command.taskId],
        p = unit(state, o.participantId),
        r = o.result;
      speaker = p.name;
      flavor = `${escapeHTML(next.name)} · ${escapeHTML(command.taskId.startsWith('stunt:') ? 'Stunt' : o.action)}<br>${r.total} vs ${r.target} · ${r.success ? 'Success' : 'Failure'} · margin ${r.margin}<br><small>Result recorded. Required consequences appear in the tracker.</small>`;
    }
    try {
      await roll.toMessage(
        {
          speaker: { alias: speaker },
          flavor: `<strong>${flavor}</strong><br><small>Requested by ${escapeHTML(user.name)}.</small>`,
        },
        { rollMode: 'publicroll' },
      );
    } catch (err) {
...
export function installStoreHooks(refresh, open = () => {}) {
  ...
  Hooks.on('createChatMessage', (message, options, creatorId) => {
    ...
    if (invitation && game.users.get(creatorId)?.isGM && creatorId !== game.user.id) {
      const doc = game.journal.get(invitation.docId);
      if (doc?.testUserPermission(game.user, 'OBSERVER')) open(doc.id);
    }
    ...
    void queue(async () => {
      ...
      try {
        ...
        content = `<p><strong>Chase:</strong> ${escapeHTML(request.command.type)} accepted for ${escapeHTML(readChase(doc).name)}.</p>`;
        ok = true;
      } catch (err) {
        error = err.message;
        content = `<p><strong>Chase request rejected:</strong> ${escapeHTML(err.message)}</p>`;
      }
  ...
  Hooks.on('updateChatMessage', (message, change, options, updaterId) => {
    ...
    for (const entry of pending.values())
      if (
        entry.nonce === receipt.nonce &&
        request?.nonce === entry.nonce &&
        entry.docId === request.docId
      )
        entry.finish(
          receipt.ok,
          receipt.error || 'The GM rejected this action. Your entries have been kept.',
        );
    refresh();
  });
  Hooks.on('renderChatMessageHTML', (message, html) => {
    const root = html instanceof HTMLElement ? html : html?.[0];
    root?.querySelectorAll('[data-gac-open]').forEach((b) =>
      b.addEventListener('click', () => {
        const doc = game.journal.get(b.dataset.gacOpen);
        if (doc?.testUserPermission(game.user, 'OBSERVER')) open(doc.id);
      }),
    );
  });
