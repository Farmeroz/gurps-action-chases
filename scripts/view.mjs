...
export const input = (name, label, value = '', type = 'text', extra = '') =>
  `<label class="gac-field"><span>${e(label)}</span><input name="${e(name)}" type="${type}" value="${e(value ?? '')}" ${extra}></label>`;
export const select = (name, label, choices, value) =>
  `<label class="gac-field"><span>${e(label)}</span><select name="${e(name)}">${choices.map((x) => option(x.value, x.label, value)).join('')}</select></label>`;
export const checkbox = (name, label, checked = false) =>
  `<label class="gac-check"><input type="checkbox" name="${e(name)}" ${checked ? 'checked' : ''}><span>${e(label)}</span></label>`;
export const textarea = (name, label, value = '') =>
  `<label class="gac-field gac-full"><span>${e(label)}</span><textarea name="${e(name)}" rows="3">${e(value)}</textarea></label>`;
const button = (action, label, attrs = '', disabled = false) =>
  `<button type="button" data-do="${action}" ${attrs} ${disabled ? 'disabled' : ''}>${label}</button>`;
const title = (side) => (side === 'quarry' ? 'Quarry' : 'Pursuer');
const control = (s, p, u) => canControl(s, p, u, actorOwned);
const PHASES = {
  quarry: 'Quarry declares',
  pursuer: 'Pursuer responds',
  actions: 'Resolve actions',
  chase: 'Chase Rolls',
  reverseCheck: 'Resolve failed Reverse',
  range: 'Choose outcome',
  ended: 'Chase ended',
};
const owner = (s, p) => {
  const names = [...(globalThis.game?.users ?? [])]
    .filter((u) => !u.isGM && control(s, p, u))
    .map((u) => u.name);
  return names.join(', ') || 'GM';
};
function taskCard(s, t, user) {
  const reason = taskAccess(s, t, user, actorOwned),
    p = unit(s, t.participantId);
...
function nextPanel(s, user) {
  const gm = user.isGM;
  let heading = 'Next action',
    body = '',
    buttons = '';
  const drivers = tasks(s).filter((t) => t.kind === 'driver');
  if (drivers.length && ['running', 'paused'].includes(s.status))
    return `<section class="gac-next"><div><span class="gac-kicker">Next action</span><h2>Resolve vehicle control</h2><p>The GM chooses a replacement driver or resolves the wreck before play continues.</p></div></section><div class="gac-tasks">${drivers.map((t) => taskCard(s, t, user)).join('')}</div>`;
  if (s.outcomeRequired)
    return `<section class="gac-next"><div><span class="gac-kicker">Next action</span><h2>Resolve this round’s outcome</h2><p>${e(s.outcomeRequired)}</p></div>${gm ? button('override', 'GM outcome / end chase', 'class="gac-primary"') : ''}</section>`;
  if (s.status === 'setup') {
    const missing = readiness(s);
    body = missing.length
      ? missing.map(e).join('<br>')
      : 'Both sides are ready. Check player assignments, then start.';
    buttons = gm
      ? missing.length
        ? button('addCharacter', 'Add character') + button('addVehicle', 'Add vehicle')
        : button('start', 'Start chase', 'class="gac-primary"')
      : 'Ask the GM to finish setup and assign your participants.';
  } else if (s.status === 'paused') {
    heading = 'Chase paused';
    body = 'Resolve tactical combat or make your changes. Resume returns to the same chase step.';
    buttons = gm ? button('resume', 'Resume chase', 'class="gac-primary"') : '';
    if (s.checks.some((c) => c.reverse && c.status === 'wreck')) {
      heading = 'Reverse ended in a wreck';
      body =
        'Choose a GM outcome to finish this round or end the chase. Any replacement leader will be used next round.';
      buttons = gm ? button('override', 'GM outcome / end chase', 'class="gac-primary"') : '';
    }
  } else if (s.status === 'ended') {
    heading = 'Chase ended';
    body = 'The result and log remain available below.';
    buttons = gm ? button('restart', 'Return to setup') : '';
  } else if (['quarry', 'pursuer'].includes(s.phase)) {
    const sides = s.blind ? SIDES.filter((k) => !s.blindRequests[k]) : [s.phase];
    if (!sides.length) {
      body = 'Both private choices are committed.';
      buttons = gm
        ? button('reveal', 'Reveal manoeuvres', 'class="gac-primary"')
        : 'Waiting for the GM to reveal.';
    } else {
      body = sides
        .map(
          (k) =>
            `${e(owner(s, unit(s, s.sides[k].leader)))}: choose ${title(k).toLowerCase()} manoeuvre below.`,
        )
        .join('<br>');
      if (s.blind) body += '<br>Each side commits privately; the GM reveals both together.';
    }
  } else if (s.phase === 'actions') {
    const all = tasks(s),
      group = activeGroup(s),
      required = remainingRequired(s),
      current = all.filter((t) => !t.done && t.group === group);
    heading =
      group === 'prepare'
        ? 'GM scene decisions'
        : group
          ? `${title(group)} actions`
          : 'Ready for Chase Rolls';
    body = required.length
      ? `${required.length} required ${required.length === 1 ? 'task remains' : 'tasks remain'}. Resolve the highlighted tasks; later tasks are listed underneath.`
      : 'All required tasks are complete. Take any optional actions now, or continue and pass them.';
    const missing = readiness(s);
    if (missing.length) {
      heading = 'Resolve the chase outcome';
      body =
        'A side has no active independent participants. Resolve any consequences below, then use a GM outcome ruling to end or reorganise the chase.';
    }
    if (!gm && current.length && !current.some((t) => !taskAccess(s, t, user, actorOwned)))
      body =
        'Waiting for ' +
        (group === 'prepare'
          ? 'the GM’s scene decisions'
          : `${title(group).toLowerCase()} actions`) +
        '. Your later actions are listed below.';
    buttons = gm
      ? required.length
        ? current.length && current.every((t) => !t.required)
          ? button('passSide', `Pass unused ${group} actions`)
          : ''
        : button(
            'actionsDone',
            all.some((t) => !t.done) ? 'Continue · pass unused actions' : 'Continue to Chase Rolls',
            'class="gac-primary"',
          )
      : '';
    if (gm && missing.length)
      buttons = button('override', 'GM outcome / end chase', 'class="gac-primary"');
    const visibleCurrent = gm
        ? current
        : current.filter((t) => !t.gm && control(s, unit(s, t.participantId), user)),
      otherCurrent = current.filter((t) => !visibleCurrent.includes(t));
    return `<section class="gac-next" aria-label="Next action"><div><span class="gac-kicker">${gm ? 'Next action' : 'Your next action'}</span><h2>${e(heading)}</h2><p>${body}</p></div><div class="gac-buttons">${buttons}</div></section><div class="gac-tasks">${visibleCurrent.map((t) => taskCard(s, t, user)).join('')}${otherCurrent.length ? `<details data-persist="others-current"><summary>Waiting on others · ${otherCurrent.length} current ${otherCurrent.length === 1 ? 'task' : 'tasks'}</summary>${otherCurrent.map((t) => taskCard(s, t, user)).join('')}</details>` : ''}${
      all.some((t) => !t.done && t.group !== group)
        ? `<details data-persist="upcoming"><summary>Upcoming tasks · ${all.filter((t) => !t.done && t.group !== group && t.required).length} required, ${all.filter((t) => !t.done && t.group !== group && !t.required).length} optional</summary>${all
            .filter((t) => !t.done && t.group !== group)
            .map((t) => taskCard(s, t, user))
            .join('')}</details>`
        : ''
    }${
      all.some((t) => t.done)
        ? `<details data-persist="completed"><summary>Completed this round · ${all.filter((t) => t.done).length}</summary><ul>${all
            .filter((t) => t.done)
            .map(
              (t) =>
                `<li>${e(t.title)} · ${t.waived ? 'GM ruling' : s.workflow.outcomes[t.id]?.passed ? 'passed' : 'recorded'}</li>`,
            )
            .join('')}</ul></details>`
        : ''
    }</div>`;
  } else if (s.phase === 'chase') {
    body = SIDES.filter((k) => !s.sides[k].roll)
      .map(
        (k) =>
          `${e(owner(s, unit(s, s.sides[k].leader)))}: roll for ${title(k).toLowerCase()} below.`,
      )
      .join('<br>');
  } else if (s.phase === 'reverseCheck') {
    body =
      'The quarry failed Reverse. Resolve the wipeout using the saved Chase Roll; there is no reroll.';
    buttons = gm
      ? `<span data-id="${e(s.sides.quarry.leader)}">${button('check', 'Resolve original result', 'class="gac-primary"')}</span>`
      : '';
  } else if (s.phase === 'range' && s.result) {
    const who = s.result.type === 'hideWin' ? 'quarry' : s.result.winner,
      can = gm || (who && control(s, unit(s, s.sides[who].leader), user));
    body = `${who ? `${title(who)} chooses the outcome` : 'GM confirms unchanged range'}. Contest margin ${s.result.margin}.`;
    buttons = rangeChoices(s, s.result)
      .map((c) => button('range', e(c.label), `data-choice="${c.key}"`, !can))
      .join('');
  }
...
function teamCard(s, side, user, docId) {
  const t = s.sides[side],
    p = unit(s, t.leader),
    d = t.declaration,
    can = control(s, p, user),
    running = s.status === 'running';
  const declares =
    running &&
    !d &&
    !s.blindRequests[side] &&
    (s.blind ? ['quarry', 'pursuer'].includes(s.phase) : s.phase === side);
  let b = null,
    error = '';
  try {
    if (d) b = breakdown(s, side);
  } catch (err) {
    error = err.message;
  }
  ...
  ${
    declares && can && !missingDriver(s, p)
      ? `<div class="gac-quick"><small>${s.blind ? 'Commit privately' : 'Choose a manoeuvre'}</small><div class="gac-buttons">${[
          'move',
          'attack',
          'moveAttack',
        ]
          ...
          .map((k) =>
            button('quickDeclare', e(MANOEUVRES[k].name), `data-side="${side}" data-key="${k}"`),
          )
          .join(
            '',
          )}${button('declare', 'More manoeuvres…', `data-side="${side}"`)}</div>${p?.kind === 'vehicle' && !manoeuvreError(s, side, 'moveAttack') ? `<label class="gac-check"><input type="checkbox" data-operator="${side}">Driver also attacks with Move and Attack</label>` : ''}</div>`
      : ''
  }
...
function participantRow(s, p, user) {
  const op = operatorOf(s, p),
    stats = travelStats(s, p),
    transport = unit(s, p.transportId),
    can = control(s, p, user);
  ...
  ${s.pendingCrew
    ...
    .map(
      (x) =>
        `<p class="gac-recorded">Next round: ${x.vehicleId ? `${e(unit(s, x.vehicleId)?.name)} · ${x.role === 'operator' ? 'driver' : 'passenger'}` : 'on foot'}</p>`,
    )
    .join('')}
...
function roster(s, user, onlyMine = false) {
  const include = (p) =>
    !onlyMine ||
    control(s, p, user) ||
    s.participants.some((x) => x.transportId === p.id && control(s, x, user));
  ...
  return (
    top
      .map(
        (p) =>
          `<div class="gac-unit">${participantRow(s, p, user)}${
            p.kind === 'vehicle'
              ? `<div class="gac-crew">${s.participants
                  .filter((x) => x.transportId === p.id)
                  .map((x) => participantRow(s, x, user))
                  .join('')}</div>`
              : ''
          }</div>`,
      )
      .join('') ||
    '<p class="gac-muted">No participants assigned to you. Ask the GM to assign control, or use an actor you own.</p>'
  );
}
function recoveryView(s, documents, selectedId, user, gmOnline, status) {
  const gm = user.isGM,
    round = s.recovery.kind === 'round';
...
export function renderTracker({
  ...
  const s = state ? upgrade(state) : null,
    gm = user.isGM,
    status = submissionStatus(selectedId);
  ...
  ${
    !s
      ? `<div class="gac-empty"><h1>Keep the chase moving.</h1><p>Add people or vehicles. Actors and tokens are optional.</p>${gm ? button('demo', 'Create example chase', 'class="gac-primary"') + button('new', 'New chase') : '<p>Your GM can open a chase for you.</p>'}</div>`
      : `<div class="gac-title"><div><h1>${e(s.name)}</h1><p>${BANDS[s.band]} range · Round ${s.round} · ${s.status === 'running' ? PHASES[s.phase] : e(s.status)}${s.combatId ? ' · Combat linked' : ''}</p></div>${gm && s.status === 'running' ? button('pause', 'Pause / tactical combat') : ''}</div>${s.environment ? `<p class="gac-muted">${e(s.environment)}</p>` : ''}
  ...
  <details class="gac-log" data-persist="log"><summary>Chase log · ${s.log.length} entries</summary><ol>${
    s.log
      .slice()
      .reverse()
      .map((x) => `<li><span>R${x.round}</span> ${e(x.text)}</li>`)
      .join('') || '<li>Ready for setup.</li>'
  }</ol></details><footer>Action 2: Exploits, pp. 31–35. Abstract chase rounds; the GM controls tactical turns, token movement and damage. Shared stats are snapshots.</footer>`
  }</div>`;
}
export function participantForm(s, p, users) {
  const onFoot = p.kind === 'character' && p.footMode && !p.transportId;
  const basic = [
    input('name', 'Name', p.name, 'text', 'required maxlength="100"'),
    select(
      'side',
      'Side',
      SIDES.map((x) => ({ value: x, label: title(x) })),
      p.side,
    ),
    input(
      onFoot ? 'footSkillName' : 'skillName',
      p.kind === 'vehicle' ? 'Fallback operator skill' : 'Chase skill',
      onFoot ? p.footSkillName : p.skillName,
    ),
    input(
      onFoot ? 'footSkill' : 'skill',
      'DX-based skill level',
      onFoot ? p.footSkill : p.skill,
      'number',
      'required min="1" max="1000"',
    ),
    input(
      'speed',
      p.kind === 'vehicle' ? 'Top speed (yards/second)' : 'Top Move (including Enhanced Move)',
      p.speed,
      'number',
  ...
  if (p.kind === 'vehicle')
    basic.push(input('handling', 'Handling', p.handling, 'number', 'required min="-20" max="20"'));
  const advanced = [];
  if (p.kind === 'vehicle')
    advanced.push(
      input(
        'sr',
        'Stability Rating (needed for failed control / stunt)',
        p.sr,
        'number',
        'min="0" max="100"',
      ),
      input('sm', 'Size Modifier', p.sm, 'number'),
    );
  else
    advanced.push(
      select(
        ...
        'Vehicle',
        [
          { value: '', label: 'Independent / on foot' },
          ...s.participants
            .filter((x) => x.kind === 'vehicle')
      ...
      select(
        ...
        [
          ...
          { value: 'operator', label: 'Driver / operator' },
        ],
        p.role,
      ),
      ...['stealth', 'acrobatics', 'climbing', 'jumping'].map((k) =>
        input(k, `${k[0].toUpperCase() + k.slice(1)} (DX-based)`, p[k], 'number'),
      ),
      checkbox('gunslinger', 'Gunslinger', p.gunslinger),
      input(
        onFoot ? 'skillName' : 'footSkillName',
        onFoot
          ? 'Vehicle / operator skill (retained)'
          : 'On-foot chase skill (for transport changes)',
        onFoot ? p.skillName : p.footSkillName || 'Running (DX-based)',
      ),
      input(
        onFoot ? 'skill' : 'footSkill',
        onFoot ? 'DX-based vehicle / operator level' : 'DX-based level on foot (optional)',
        onFoot ? p.skill : p.footSkill,
        'number',
        'min="1" max="1000"',
      ),
    );
  advanced.push(
    ...[
      ...
      ['dr', 'DR'],
      ['ht', 'HT'],
    ].map(([k, l]) => input(k, l, p[k], 'number')),
    select(
      'actorUuid',
      'Linked actor (optional)',
      [
        { value: '', label: 'Ad hoc; no actor needed' },
        ...[...(globalThis.game?.actors ?? [])].map((a) => ({ value: a.uuid, label: a.name })),
        ...(p.actorUuid && ![...(globalThis.game?.actors ?? [])].some((a) => a.uuid === p.actorUuid)
          ? [{ value: p.actorUuid, label: 'Current linked actor / token' }]
          : []),
      ],
      p.actorUuid,
    ),
    textarea('notes', 'Shared notes', p.notes),
  );
  return `<p>Use a DX-based skill level. ${p.kind === 'vehicle' ? 'A separate driver replaces the fallback skill; the vehicle supplies speed and Handling.' : 'Convert HT-based Running to DX for Chase Rolls.'} Values stay with the chase.</p><div class="gac-form-grid">${basic.join('')}</div><fieldset><legend>Who controls this participant?</legend><p>Linked actor owners can act automatically. Assign any player below, including for ad hoc entries.</p>${
    users
      .filter((u) => !u.isGM)
      .map((u) => checkbox(`controller:${u.id}`, u.name, p.controllers.includes(u.id)))
      .join('') || '<p>No player accounts yet. The GM can control everyone.</p>'
  }</fieldset><details data-advanced ${p.transportId ? 'open' : ''}><summary>Optional stats, crew, actor link & notes</summary><div class="gac-form-grid">${advanced.join('')}</div></details>`;
}
...
export function combatChoices(combats, selectedId = '') {
  ...
  const names = list.map((c, i) => {
    ...
    return (
      (name && name !== c.id && name !== `Combat ${c.id}` ? name : '') ||
      c.scene?.name ||
      `Encounter ${i + 1}`
    );
  });
  ...
  if (selectedId && !list.some((c) => c.id === selectedId))
    options.push({ value: selectedId, label: 'Previously linked encounter (unavailable)' });
...
export function settingsForm(s, combats) {
  ...
  const rules = setup
    ? `<details><summary>Rules choices</summary><p>Choose how this chase handles two ambiguities in the printed rules. These choices are saved with the chase and lock at Start chase.</p>${select(
        'speedRounding',
        'Speed bonus at Move 6',
        [
          { value: 'action', label: 'Action 2 example: +3' },
          { value: 'table', label: 'Round down on the Size table: +2' },
        ],
        ruleOptions.speedRounding,
      )}${select(
        'driverShots',
        'Driver ranged attacks during Force, Ram and Disembark / Embark',
        [
          { value: 'general', label: 'Allow: general Attacks text (p. 35)' },
          { value: 'passengers', label: 'Passengers only: individual manoeuvre text' },
        ],
        ruleOptions.driverShots,
      )}<p>Gunslinger keeps its stated exceptions. These options do not change the manoeuvre’s contact attacks or Chase Roll penalty.</p></details>`
    : `<details><summary>Rules choices · fixed for this chase</summary><p>Move 6 speed bonus: ${ruleOptions.speedRounding === 'table' ? '+2 (table rounding)' : '+3 (Action 2 example)'}. Driver ranged attacks during Force, Ram and Disembark / Embark: ${ruleOptions.driverShots === 'passengers' ? 'passengers only, with Gunslinger exceptions' : 'allowed under general Attacks text'}.</p></details>`;
  const name = input('name', 'Name', s.name, 'text', 'required maxlength="100"');
  const status = setup
    ? `<div class="gac-form-grid">${name}${select(
        'band',
        'Starting range',
